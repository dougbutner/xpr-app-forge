...
## This repo only (extras; not in upstream xpr-network-dev-skill)

Synced from [XPRNetwork/xpr-network-dev-skill](https://github.com/XPRNetwork/xpr-network-dev-skill) **v2.3.2**. These modules are kept in addition:

| Module | Read When | Key Topics |
|--------|-----------|------------|
| `alcor-exchange.md` | Alcor `/v/xpr/` URL index | Local Alcor module map + web vs API hosts |
| `alcor-overview-ui.md` | Alcor UI / fees | Product overview, fees, community |
| `alcor-orderbook.md` | Alcor order book | Tables, memos, RPC patterns |
| `alcor-swap-amm.md` | Alcor AMM | `swap.alcor`, CL, TWAP |
| `alcor-api-realtime.md` | Alcor API | REST + Socket.IO |
| `alcor-widgets-ecosystem.md` | Embeds / deep links | Widgets, farms, **`https://alcor.exchange/v/xpr/...`** |
| `flextokens.md` | Flextokens / EASY | Flex token patterns for this template |
| `README.md` | Local map | Points here + lists extras |

For Alcor: start with upstream **`alcor-dex.md`**, then use the local `alcor-*.md` split for **`/v/xpr/`** link rules. Cursor Agent Skills under **`.agents/skills/`** wrap these modules.
